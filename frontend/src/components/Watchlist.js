...
function Watchlist() {
    ...
    async function removeWatchlistItem(idx, id) {
                        ...
                        <div className="mt-2">
                            {watchlistitems.map((item, i) => (
                                <div 
                                    ...
                                    className={`row trigger ${item.symbol === activeStock ? 'activeStock' : ''}`} 
                                    id={item.symbol}
                                >
                                    <div className="col-3">{item.symbol}</div>
                                    <div className="col-7">{companyName(item.company)}</div>
                                    <div className="col-2 remove-wl-button" onClick={() => removeWatchlistItem(i, item.id)}>
